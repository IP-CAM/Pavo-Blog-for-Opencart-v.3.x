...
class ModelExtensionPavoBlogComment extends Model {
	...
	public function getComments( $post_id = null ) {
		...
		foreach ( $rows as $k => $row ) {
			if ( isset( $row['comment_user_id'] ) && $row['comment_user_id'] ) {
				$image = $this->getUserImage( $row['comment_user_id'] );
				if ( ! $image ) {
					$image = 'no_image.png';
				}
	...
	public function addComment( $data = array() ) {
		...
		extract( $data );
		$sql = "INSERT INTO " . DB_PREFIX . "pavoblog_comment ( `comment_title`, `comment_name`, `comment_email`, `comment_text`, `comment_post_id`, `comment_user_id`, `comment_parent_id`, `comment_customer_id`, `comment_status`, `comment_subscribe`, `comment_store_id`, `comment_language_id`, `date_added`, `date_modified` )";
		$sql .= " VALUES( '".$this->db->escape( $comment_title )."', '".$this->db->escape( $comment_name )."', '".$this->db->escape( $comment_email )."', '".$this->db->escape( $comment_text )."', ".(int)$comment_post_id.", ".(int)$comment_user_id.", ".(int)$comment_parent_id.", ".(int)$comment_customer_id.", ".(int)$comment_status.", ".(int)$comment_subscribe.", ".(int)$comment_store_id.", ".(int)$comment_language_id.", NOW(), NOW() )";
		...
		// subscribe post
		if ( $comment_id && $post_subscribe && $comment_email ) {
			$sql = "SELECT * FROM " . DB_PREFIX . "pavoblog_subscribe_post WHERE subscribe_email = '".$this->db->escape( $comment_email )."'";
			$query = $this->db->query( $sql );

			if ( ! $query->row ) {
				$sql = "INSERT INTO " . DB_PREFIX . "pavoblog_subscribe_post ( `subscribe_email` ) VALUES( '".$this->db->escape( $comment_email )."' )";
	...
	public function updateComment( $data = array() ) {
		...
		// delete subscribe
		if ( ! $comment_subscribe && $comment_email ) {
			$sql = "DELETE * FROM " . DB_PREFIX . "pavoblog_subscribe_post WHERE subscribe_email = '".$this->db->escape( $comment_email )."'";
			$this->db->query( $sql );
		}

		// update comment
		$sql = "UPDATE " . DB_PREFIX . "pavoblog_comment SET `comment_name` = '".$this->db->escape( $comment_name )."', `comment_email` = '".$this->db->escape($comment_email)."', `comment_title` = '".$this->db->escape($comment_title)."', `comment_subscribe` = ".(int)$comment_subscribe.", `comment_text` = '".$this->db->escape($comemnt_text)."'";
